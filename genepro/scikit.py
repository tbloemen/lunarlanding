...
class GeneProEstimator(BaseEstimator):
    def __init__(
        ...
        self.evo = Evolution(fitness_function=self.score, **evo_kwargs)
...
class GeneProRegressor(GeneProEstimator):
    ...
    def predict(self, X, best_ever=False):
        ...
        if best_ever:
            best = self.evo.best_of_gens[
                np.argmax([t.fitness for t in self.evo.best_of_gens])
            ]
        else:
...
class GeneProClassifier(GeneProEstimator):
    ...
    def fit(self, X, y):
        ...
        if len(self.classes_) > 2:
            raise ValueError(
                "Only binary classification is supported (repeat a one vs. all approach for multi-class)"
            )
