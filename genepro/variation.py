...
def generate_random_multitree(
    n_trees: int, internal_nodes: list, leaf_nodes: list, max_depth: int
):
    multitree = Multitree(n_trees)
    for _ in range(n_trees):
        multitree.children.append(
            generate_random_tree(internal_nodes, leaf_nodes, max_depth, curr_depth=0)
        )
    return multitree


def generate_random_tree(
    internal_nodes: list, leaf_nodes: list, max_depth: int, curr_depth: int = 0
):
    """
...
def subtree_crossover(
    multitree: Multitree, multidonor: Multitree, unif_depth: int = True
) -> Multitree:
    """
    ...
    for n in nodes:
        if randu() < prob_swap:
            ...
            if same_depth:
                depth = n.get_depth()
                compatible_nodes = (
                    donor_node_arity_n_depth[(arity, depth)]
                    if (arity, depth) in donor_node_arity_n_depth
                    else None
                )
            else:
                compatible_nodes = (
                    donor_node_arity[arity] if arity in donor_node_arity else None
                )
...
def subtree_mutation(
    ...
    internal_nodes: list,
    leaf_nodes: list,
    unif_depth: bool = True,
    max_depth: int = 4,
    prob_leaf: float = 0.25,
) -> Node:
    """
...
def coeff_mutation(
    multitree: Multitree, prob_coeff_mut: float = 0.25, temp: float = 0.25
) -> Node:
    """
...
def generate_offspring(
    ...
    for i in random_order:
        ...
        offspring = __undergo_variation_operator(
            var_op,
            offspring,
            crossovers,
            mutations,
...
def __undergo_variation_operator(
    var_op: dict,
    offspring: Node,
    crossovers,
    mutations,
...
def __check_tree_meets_all_constraints(
    multitree: Multitree, constraints: dict = dict()
) -> bool:
    """ """
    meets = True
    for tree in multitree.children:
        for constraint_name in constraints.keys():
            if constraint_name == "max_tree_size":
                ...
                raise ValueError(
                    "Unrecognized constraint name: {}".format(constraint_name)
                )
    return meets
